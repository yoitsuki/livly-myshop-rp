...
src/
  ...
  components/
    ...
    InputActions.tsx        各種 input の **右内側に absolute 配置** する「ペースト 📋 / クリア × 」ゴーストアイコンセット (v0.27.18 で初版、 v0.27.20 で内側オーバーレイに刷新) 。 navigator.clipboard.readText() 経由でペースト、 digitsOnly フラグで数値専用 input ( 最低販売価格 等 ) では非数値を除去。 呼出側は `<div className="relative">` で input をラップし、 input className に `pr-20` ( = 64px ) を足してボタン領域を空ける。 tabIndex=-1 で tab フォーカスから外す
