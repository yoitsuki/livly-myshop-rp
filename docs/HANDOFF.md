...
最新: **0.6.1**（commit `d27e2d9` をマージ済 / `f18a56c`）

直近のチェンジログ要約:
- 0.6.x — マイショップ参考価格を**複数件**保持（`priceEntries[]`）。`minPrice` だけは Item 直下（時期によって変わらないため）。詳細画面に履歴一覧 + 価格追加/編集ルート。価格エントリのフォームは画像プレビュー + 参考価格のみ OCR。Dexie v4 まで進み、移行は **wipe**（ローンチ前のため過去データ捨て可）
- 0.5.x — 期間バッジを彩度ダウン系グラデーションに変更／編集画面のクロップを **per-slot + ステージング**化（保存まで DB 反映なし）／メイン画像削除もステージング化／単一トランザクション化で Blob 再 put エラーを回避／情報元を「なんおし／その他」のバッジ化
