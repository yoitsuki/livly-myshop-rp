...
`pnpm lint` で baseline **26 件 ( 22 errors + 4 warnings )** が出るが、いずれも v0.10.0 以前から存在するパターン: 主に `react-hooks/set-state-in-effect` ( `URL.createObjectURL` を effect 内で `setState` する既存ロジック / Toast の open → mounted 反映 / useItem の id 切替時 setData / localSettings の storage event 受け、 etc. ) と `react-hooks/refs` ( ImageCropper の crop 中 ref アクセス ) と `react-hooks/exhaustive-deps` ( register/tags page の useEffect 依存 ) 。 React 19 の strict 化対応として将来別ラウンドで `useSyncExternalStore` 等に書換える余地あり。 Vercel build (`next build`) は通るので公開には支障なし。 v0.27.0 統合の前後で件数変化なし ( origin/main と一致 ) 。
