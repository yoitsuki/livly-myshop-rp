...
### 細かい改善候補
- **レプリカ管理** — 同じ見た目で「原本 / レプリカ」が区別されるアイテムがあり、それを記録 + 検索したい。**用語注意: ゲーム内呼称は「原本」 ( 「本物」ではない ) — UI ラベル / コミットメッセージ / コメント全部 `原本` 表記で統一**。要望:
  - 登録 / 編集フォームに「レプリカ」チェックボックスを追加
  - チェックを付けたアイテムは内部的に「レプリカ」として保持 ( `Item` に `isReplica?: boolean` を追加 → `itemFromFs` / `itemToFs` のマッパー更新 )
  - ホームのフィルタで「レプリカのみ / 原本のみ / 両方 (既定)」のトグルを置いて検索できるようにする ( タグのフィルタ列の近くか、ヘッダ側のセグメント )
  - 詳細カードに小さい「REPLICA」バッジ ( Atelier 流に枠線 + uppercase )
  - bulk 行 / inbox 行に小さなチェックを入れるかは UI 密度を見て判断 ( v1 ではフォームでの設定のみで足りるかも )
  - 既存ドキュメントは `isReplica` を持たないので、欠損 = false ( = 原本 ) 扱いで OK ( マイグレーション不要 )
- **ホームのタグフィルタ密度** — v0.16.0 で section 化 (TYPE_LABEL 見出し付き) → v0.16.3 で件数表示 + 0件タグ非表示まで進めたが、SEED 58 件中のアクティブタグが多いとまだファーストビューが埋まりがち。**ユーザーから「後で考えよう」とコメント済**。残候補: セクションごと折りたたみ / 既定で「期間 (グレショ・バザール) のみ展開」/ "もっと見る" リンク / よく使うタグだけ pin 表示など。手を入れる時は実機で密度を確認しながら
