...
### 細かい改善候補
- **ホームのタグフィルタ密度** — v0.16.0 で section 化 (TYPE_LABEL 見出し付き) したが、SEED 58 件投入後はファーストビューがタグだらけになる。**ユーザーから「後で考えよう」とコメント済**。候補: セクションごと折りたたみ / 既定で「期間 (グレショ・バザール) のみ展開」/ "もっと見る" リンク / よく使うタグだけ pin 表示など。手を入れる時は実機で密度を確認しながら
- **`ConfirmDialog` の primitives 切り出し** — `items/[id]/page.tsx` の `ConfirmDialog` と `register/page.tsx` の `MergeDialog` (v0.16.1) はほぼ同じ造り (中央寄せ・ハイライト無しの白カード + 2 ボタン)。`tags` / `presets` ページの削除確認も `window.confirm()` のままなので、`src/components/ui/Modal.tsx` 等に共通化すると 3 箇所まとめて整理できる
